...
class Itv extends AjaxResponse implements \Stalker\Lib\StbApi\Itv {
    ...
    public function getRealChannelByChannelId($ch_id, $link_id = null) {
        ...
        if ($channel['use_http_tmp_link']) {
            ...
            } else if (!empty($link) && $link['wowza_securetoken']){
            ...
            } else if ($channel['flexcdn_auth_support']){
    ...
    private function getFlexCDNToken($url) {
        ...
        if (strpos($url, $test_prefix) !== false) {
            # only API call if url for Channel include TestPrefix above
    ...
    public static function getNginxSecureLink($cmd) {
        ...
        if (preg_match("/:\/\/([^\/]+)\/?(\S*)/", $cmd, $match)) {
            ...
            if (strpos($nginx_secure_link_order, '$uri') !== FALSE) {
                ...
                if(substr(strtolower($url_path), -5) == '.m3u8'){                           //todo: ? ..here to remove the extension from the playlist (.m3u8 file) and from the HLS segments (.ts files) issue #17019
...
// egdecast code
    ...
    public static function getWowzaSecureToken($url, $endtime = null){
        ...
        $hashing_string .= '?';
        $secret_hashing_params = array();
        $query_params = array();

        $prefix = Config::getSafe('wowza_securetoken_query_prefix', 'wowzatoken');

        if ($endtime === null){
            $endtime = Config::getSafe('wowza_securetoken_tv_endtime', 0);
        }

        $shared_secret = Config::getSafe('wowza_securetoken_shared_secret', 'xyzSharedSecret');

        if (Config::getSafe('wowza_securetoken_include_client_ip', false)){
            $secret_hashing_params[] = Stb::getInstance()->ip;
        }

        $secret_hashing_params[] = $shared_secret;

        $query_params[] = $prefix.'endtime='.($endtime == 0 ? '0' : time() + $endtime);

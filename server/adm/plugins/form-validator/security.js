  ...
  var requestServer = function (serverURL, $element, val, conf, callback) {
      ...
      $.ajax({
        ...
        error: function (error) {
          handleResponse({valid: false, message: error.responseJSON && error.responseJSON.message ? error.responseJSON.message : 'Connection failed with status: ' + error.statusText }, callback);
          return false;
        },
  ...
  $.formUtils.addAsyncValidator({
    ...
    validatorFunction: function (done, val, $input, conf, lang, $form) {
      ...
      requestServer(serverURL, $input, val, conf, function (response) {
        ...
        if (response.message) {
          $input.attr(conf.validationErrorMsgAttribute + '-server', response.message);
        }
        done(response.valid);
      });
    },

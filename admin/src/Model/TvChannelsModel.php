...
class TvChannelsModel extends \Model\BaseStalkerModel {
    ...
    public function getAllChannels($param = array(), $counter = FALSE) {
        ...
        if ((!empty($param['in']) && is_array($param['in'])) || (!empty($param['not_in']) && is_array($param['not_in']))) {
            foreach (array('in', 'not_in') as $in){
                if (array_key_exists($in, $param)) {
                    $not = ($in === 'not_in');
                    foreach ($param[$in] as $field => $values){
                        $this->mysqlInstance->in($field, (is_array($values) ? $values: array($values)), $not);
                    }
                }
            }
        }
    ...
    public function insertITVChannel($data){
        ...
        $input = array(
            ...
            'cost' => (!empty($data['cost']) && is_numeric($data['cost'])? $data['cost']: 0),
        ...
        if (!empty($data['tv_archive_type']) && in_array($data['tv_archive_type'], array('wowza_dvr', 'flussonic_dvr', 'nimble_dvr'))) {
    ...
    public function getStorages() {
        return $this->mysqlInstance->from('storages')->where(array('status' => 1, 'for_records' => 1, '`dvr_type` IS NOT' => NULL, '`dvr_type` <>' => ''))->where(array(' stream_server_type' => NULL, 'stream_server_type' => ''), 'OR ')->get()->all();
    }
    ...
    public function resetMediaClaims($media_id){
        return $this->mysqlInstance->update('media_claims',
            ...
            array('media_id' => intval($media_id), 'media_type' => 'itv'))->total_rows();
    }
